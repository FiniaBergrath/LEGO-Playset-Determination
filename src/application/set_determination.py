...
class BrickSetClassifier():
    def __init__(self,db):
        self.db = db
        pass

    def cross_entropy(self, detected_bricks, detected_counts, detected_total, set_bricks, set_counts, set_total, direct_matching = True):
    ...
    def sorting_key(self,cross_entropy_result):
        set_id, ce = cross_entropy_result
        return ce


    def determine_matching_of_sets(self, detected_bricks, detected_counts, sets):
        ...
        for set in sets:
            name, set_id = set
            print("Set_id:", set_id)
            set_total, set_bricks, set_counts = self.db.get_set_elements(set_id)
            ...
            if(set_total == None):
                ...
                direct_ce = self.cross_entropy(detected_bricks, detected_counts, detected_total, set_bricks, set_counts, set_total)
                ...
                indirect_ce = self.cross_entropy(detected_bricks, detected_counts, detected_total, set_bricks, set_counts, set_total, direct_matching=False)
